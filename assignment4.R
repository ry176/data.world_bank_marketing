...
library(readxl)
bank = read_xlsx('C:\\Academics\\Fall 2020 courses\\multivariate analysis\\homework\\hw3\\bank.xlsx')
# use dummy variables to replace categorical variables
job<- model.matrix(~job, data = bank)[,-1]
marital = model.matrix(~marital, data = bank)[,-1]
education = model.matrix(~education, data = bank)[,-1]
default = model.matrix(~default, data = bank)[,-1]
...
summary(bank_pca)
bank_2 <- cbind(data.frame(bank$y),bank_pca$x)# add up the dependent to the score matrix
